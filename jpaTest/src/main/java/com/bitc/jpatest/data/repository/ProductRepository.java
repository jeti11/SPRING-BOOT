...
// JpaRepository 를 상속받았기 때문에 JpaRepository가 제공하는 기본 메소드와 쿼리 메소드를 생성할 수 있음
public interface ProductRepository extends JpaRepository<ProductEntity, Long> {

  // 쿼리 메소드 : CrudRepository에서 지원하는 기본 쿼리 메소드를 가지고 원하는 모든 기능을 사용할 수 없기 때문에 특정 키워드를 사용하여 사용자 정의 메소드를 생성하는 방식의 메소드
  // 주제 키워드를 조합하여 원하는 메소드를 생성하여 사용할 수 있음
  // @Query : 복잡한 쿼리문은 쿼리 메소드로 만들기 힘들기 때문에 @Query 라는 어노테이션을 제공함

  // ※ 쿼리 메소드 생성 방법
  // 리턴타입 + {주제 키워드 + 서술어} 를 조합하여 메소드 생성 ({}부분은 반복 됨)
  // ex) List<Person> findByLastnameAndEmail(String lastName, String email);

  // ※ 쿼리 메소드의 주제 키워드
  // find...By : select 명령을 수행하는 키워드, read...By, get...By, query...By, search...By, stream...By 가 추가로 존재함
  // find와 By 사이의 ...에는 entity명을 써줘야 하나, JpaRepository를 상속받을때 컬렉션 타입으로 지정을 했기 때문에 생략해도 됨
  Optional<ProductEntity> findByNumber(Long number); // 지정한 번호를 기준으로 데이터 가져오기
...
//   queryByName(String name); // 다른 키워드로 데이터 가져오기


  // exists...By : 특정 데이터가 존재하는지 확인하는 키워드, true/false
  ...
  void deleteByNumber(Long number); // 지정한 번호를 기준으로 데이터 삭제
  Long removeByName(String name); // 지정한 이름을 기준으로 데이터 삭제, 삭제된 데이터의 수 반환
  ...
  List<ProductEntity> findTop5ByNumber(Long number); // 조회된 결과 중 숫자가 가장 큰 5개 출력
...
//  And, Or : 조건을 추가하는 키워드
  ProductEntity findByNumberAndName(Long number, String name); // and 명령으로 조회 조건을 추가, 지정한 번호 및 이름과 같은 데이터를 조회
  ProductEntity findByNumberOrName(Long numeber, String name); // or 명령으로 조회 조건을 추가, 지정한 번호나 지정한 이름을 가지고 있는 데이터를 조회


  // (Is)GreaterThan, (Is)LessThan, (Is)Between : 숫자나 datetime 컬럼을 대상으로 비교 연산에 사용할 수 있는 조건자 키워드
  // GraterThan, LessThan은 비교 대상에 대한 초과 및 미만을 비교 연산을 수행
  // 경계값을 포함할 경우 Equal 키워드를 추가
  List<ProductEntity> findByPriceIsGreaterThan(int price); // 지정한 가격을 초과하는 데이터 조회
  List<ProductEntity> findByPriceGreaterThan(int price);
  List<ProductEntity> findByPriceGreaterThanEqual(int price); // 지정한 가격 이상인 데이터 조회
  List<ProductEntity> findByPriceLessThan(int price);// 지정한 가격 미만인 데이터 조회
  List<ProductEntity> findByPriceIsLessThan(int price);
  List<ProductEntity> findByPriceIsLessThanEqual(int price); // 지정한 가격 이하인 데이터 조회
  List<ProductEntity> findByPriceIsBetween(int lowPrice, int highPrice); // 지정한 낮은 가격부터 높은 가격 까지의 데이터를 조회
  List<ProductEntity> findByPriceBetween(int lowPrice, int highPrice);
  ...
  List<ProductEntity> findByNameContains(String name); // 지정한 문자열이 양끝에 포함되어 있는 이름을 조회
  ...
  List<ProductEntity> findByNameOrderByPriceAscStockDesc(String name); // 지정한 이름으로 데이터를 조회 후, 가격을 기준으로 오름차순 정렬을 먼저하고, 재고를 기준으로 내림차순 정렬
...
//  List<ProductEntity> findByName(String name); 와 같이 쿼리 메소드를 생성 후 실행 시에 Sort 객체를 추가하여 실행
  ...
  @Query("SELECT p FROM ProductEntity AS p") // p가 ProductEntity의 객체라고 생각하면 됨
  ...
  @Query("SELECT p FROM ProductEntity AS p WHERE p.name = '사인펜' ")
