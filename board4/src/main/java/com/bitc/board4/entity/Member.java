...
// @Table : 지정한 엔티티와 실제 DB의 테이블을 연동할 때 사용(주로 name 속성을 사용하여 테이블명 지정)
...
public class Member {
  ...
  // @GeneratedValue : 데이터가 추가될 경우, PK값을 자동 증가할지 여부를 지정하는 어노테이션
  // 4가지 옵션이 존재
  // -AUTO : 선택한 데이터 베이스에 따라 자동으로 선택
  // -IDENTITY : 기본 키 생성을 데이터베이스의 설정에 따름
  ...
  // 기본적으로 자바 클래스에서 카멜명명법을 사용 시 데이터베이스에서 스네이크 명명법으로 동작됨
  // - unique : 컬럼의 유니크 여부 설정
  // - nullable : 컬럼의 null 허용 여부 설정
  // - updatable : 컬럼의 수정가능 여부 설정
